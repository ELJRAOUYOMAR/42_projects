...
void *examination_routine(void *arg)
{
    ...
    data = (t_data *) arg;
    while (!emulation_finished(data))
    {
        i = 0;
        if (check_philo_death(data, &i) || check_each_philo_eat(data))
        {
            pthread_mutex_lock(&data->end_lock);
            data->emulation_end = 1;
...
int	check_philo_death(t_data *data, int *i)
{
    ...
    while (*i < data->num_philos)
    {
        ...
        currect_time = get_timestamp();
        if (currect_time == -1)
        {
            pthread_mutex_unlock(&data->meal_lock);
            return (1);
        }
        time_since_meal = time_difference(data->philosophers[*i].last_meal_time,
            currect_time);
